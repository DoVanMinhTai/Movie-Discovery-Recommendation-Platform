...
type Props = {
    ...
    onDtypeChange: (dtype: string) => void;
}
...
export default function SideBar({ genres, activeSort, activeGenre, activeDtype, onSortChange, onGenreChange, onDtypeChange }: Props) {
     const sortOptions = [
     ...
     return (
         <div className="flex flex-col gap-8 sticky top-24">
             {/* Loại */}
             <div>
                 <h3 className="text-gray-400 uppercase text-xs font-bold tracking-widest mb-4">Loại</h3>
                 <div className="flex flex-col gap-2">
                     {typeOptions.map((opt) => (
                         <button
                             key={opt.id}
                             onClick={() => onDtypeChange(opt.id)}
