...
export default function Series() {
    ...
    const dtypeParam = 'SERIES';
    const [page, setPage] = useState(0);
    const genresQuery = useQuery({ queryKey: ['genres'], queryFn: getAllGenre, staleTime: Infinity, });

    const movieQuery = useQuery({ queryKey: ['series', sortByParam, genreId, page], queryFn: () => getMoviesFilter({ sortBy: sortByParam, genre: genreId, dtype: dtypeParam, page }), placeholderData: (pre) => pre, });
    ...
    const handleDtypeChange = (newDtype: string) => { if (newDtype === 'MOVIE') { navigate('/movies'); } else if (newDtype === '') { navigate('/category'); } }
    const banner_image = "https://images.unsplash.com/photo-1522869635100-9f4c5e86aa37?q=80&w=1280";
    return (<div className="flex flex-col w-full bg-[#141414] min-h-screen text-white">
        <div className="relative w-full h-[50vh] overflow-hidden">
            ...
            <div className="absolute bottom-10 left-12">
                <h1 className="text-5xl font-bold mb-2">Phim bộ</h1>
                <p className="text-gray-300 text-lg">Khám phá các series và phim bộ hấp dẫn</p>
            </div>
        </div>
