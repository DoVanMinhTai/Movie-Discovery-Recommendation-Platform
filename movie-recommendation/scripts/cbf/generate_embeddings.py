...
class ElasticsearchCBFMigration:
    def __init__(self):
        # Tách host và port nếu ES_HOST có chứa port (ví dụ: host:443)
        raw_host = settings.ES_HOST.replace("http://", "").replace("https://", "")
        if ":" in raw_host:
    ...
    def bulk_index_movies(self, df: pd.DataFrame, index_name: str, batch_size: int = 1000):        
        ...
        def doc_generator():
            for idx, (_, row) in enumerate(df.iterrows()):
                m_id = row.get('media_content_id') or row.get('movieId') or idx
                doc = {
                    '_index': index_name,
                    '_id': str(m_id),
                    '_source': {
